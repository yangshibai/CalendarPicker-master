...
public class SimpleMonthAdapter extends RecyclerView.Adapter<SimpleMonthAdapter.ViewHolder> implements SimpleMonthView.OnDayClickListener {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        year = position / MONTHS_IN_YEAR - 2 + calendar.get(Calendar.YEAR);
    ...
    public int getItemCount() {
        return 24 + calendar.get(Calendar.MONTH) + 1;
    }
    ...
    public void setSelectedDay(CalendarDay calendarDay) {
        ...
        if (selectedDays.getFirst() != null && selectedDays.getLast() == null) {
            ...
            //第二次选择年份等于第一次选择
            if (selectedDays.getFirst().getYear() == calendarDay.year) {

                if ((selectedDays.getFirst().getMonth() == calendarDay.month)) {
                    //第二次选择月份等于第一次选择
                    if (selectedDays.getFirst().getDay() >= calendarDay.day) {
                        //第二次选择天数小于第一次选择
                ...
                } else if ((selectedDays.getFirst().getMonth() > calendarDay.month)) {
                    //第二次选择月份小于第一次选择
                    selectedDays.setFirst(calendarDay);
                    selectedDays.setLast(null);
                } else {
            ...
            } else if (selectedDays.getFirst().getYear() < calendarDay.year) {
                //第二次选择年份大于第一次选择
                selectedDays.setLast(calendarDay);
                if (selectedDays.getFirst().month < calendarDay.month) {
                    for (int i = 0; i < selectedDays.getFirst().month - calendarDay.month - 1; ++i) {
